...
/// <summary>
/// Represents a single Mos6502 instruction, including opcode and operands.
/// </summary>
...
/// <summary>
/// Represents a single Mos6510 instruction, including opcode and operands.
/// </summary>
public readonly struct Mos6510Instruction : IEquatable<Mos6510Instruction>, ISpanFormattable
{
    private readonly uint _raw;

    /// <summary>
    /// Initializes a new instance of the <see cref="Mos6510Instruction"/> struct from a raw 32-bit value.
    /// </summary>
    /// <param name="raw">The raw 32-bit value encoding the instruction.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal Mos6510Instruction(uint raw) => _raw = raw;

    /// <summary>
    /// Initializes a new instance of the <see cref="Mos6510Instruction"/> struct with the specified opcode.
    /// </summary>
    /// <param name="opCode">The opcode.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Mos6510Instruction(Mos6510OpCode opCode)
    {
        _raw = ((uint)opCode);
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="Mos6510Instruction"/> struct with the specified opcode and signed operand.
    /// </summary>
    /// <param name="opCode">The opcode.</param>
    /// <param name="lowByte">The signed operand byte.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Mos6510Instruction(Mos6510OpCode opCode, sbyte lowByte) : this(opCode, (byte)lowByte)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="Mos6510Instruction"/> struct with the specified opcode and operand.
    /// </summary>
    /// <param name="opCode">The opcode.</param>
    /// <param name="lowByte">The operand byte.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Mos6510Instruction(Mos6510OpCode opCode, byte lowByte)
    {
        _raw = ((uint)opCode | ((uint)lowByte << 8));
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="Mos6510Instruction"/> struct with the specified opcode and 16-bit operand.
    /// </summary>
    /// <param name="opCode">The opcode.</param>
    /// <param name="value">The 16-bit operand value.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Mos6510Instruction(Mos6510OpCode opCode, ushort value)
    {
        _raw = ((uint)opCode | ((uint)value << 8));
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="Mos6510Instruction"/> struct with the specified opcode and two operand bytes.
    /// </summary>
    /// <param name="opCode">The opcode.</param>
    /// <param name="lowByte">The low operand byte.</param>
    /// <param name="highByte">The high operand byte.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Mos6510Instruction(Mos6510OpCode opCode, byte lowByte, byte highByte)
    {
        _raw = ((uint)opCode | ((uint)lowByte << 8) | ((uint)highByte<< 16));
    }
    ...
    /// Gets the opcode of the instruction.
    /// </summary>
    public Mos6510OpCode OpCode => (Mos6510OpCode)(byte)_raw;

    /// <summary>
    /// Gets the mnemonic for the instruction.
    /// </summary>
    public Mos6510Mnemonic Mnemonic => OpCode.ToMnemonic();
    ...
    /// <inheritdoc/>
    public bool Equals(Mos6510Instruction other) => _raw == other._raw;

    /// <inheritdoc/>
    public override bool Equals(object? obj) => obj is Mos6510Instruction other && Equals(other);

    /// <inheritdoc/>
    public override int GetHashCode() => (int)_raw;

    /// <summary>
    /// Decodes a buffer into a <see cref="Mos6510Instruction"/>.
    /// </summary>
    /// <param name="buffer">The buffer containing the instruction bytes.</param>
    /// <returns>The decoded instruction, or default if decoding fails.</returns>
    public static Mos6510Instruction Decode(ReadOnlySpan<byte> buffer) => TryDecode(buffer, out var instruction, out var count) ? instruction : default;

    /// <summary>
    /// Tries to decode a buffer into a <see cref="Mos6510Instruction"/>.
    /// </summary>
    /// <param name="buffer">The buffer containing the instruction bytes.</param>
    /// <param name="instruction">The decoded instruction.</param>
    /// <param name="sizeInBytes">The size in bytes of the instruction.</param>
    /// <returns><c>true</c> if decoding was successful; otherwise, <c>false</c>.</returns>
    public static bool TryDecode(ReadOnlySpan<byte> buffer, out Mos6510Instruction instruction, out int sizeInBytes)
    {
        instruction = default;
        sizeInBytes = 0;
        ...
        var addressingMode = Mos6510Tables.GetAddressingModeFromOpcode(opCode);
        if (addressingMode == Mos6502AddressingMode.Unknown)
            return false;
        ...
        {
            case 1:
                instruction = new Mos6510Instruction((Mos6510OpCode)opCode);
                break;
            case 2:
                instruction = new Mos6510Instruction((Mos6510OpCode)opCode, buffer[1]);
                break;
            default:
                instruction = new Mos6510Instruction((Mos6510OpCode)opCode, buffer[1], buffer[2]);
                break;
        }
    ...
    public string ToString(string? format, IFormatProvider? formatProvider)
    {
        ...
        if (!TryFormat(destination, out var charsWritten, format, formatProvider))
        {
            ...
            if (!TryFormat(destination, out charsWritten, format, formatProvider))
            {
                throw new InvalidOperationException("Failed to format Mos6510Instruction.");
            }
        }
    ...
    /// Determines whether two instructions are equal.
    /// </summary>
    public static bool operator ==(Mos6510Instruction left, Mos6510Instruction right) => left.Equals(right);
