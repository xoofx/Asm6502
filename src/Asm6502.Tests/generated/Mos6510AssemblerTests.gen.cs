// Copyright (c) Alexandre Mutel. All rights reserved.
// Licensed under the BSD-Clause 2 license.
// See license.txt file in the project root for full license information.
// ------------------------------------------------------------------------------
// This code was generated by Asm6502.CodeGen.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// ------------------------------------------------------------------------------
// ReSharper disable All
#pragma warning disable CS1573
using System.Runtime.CompilerServices;
using static Asm6502.Mos6502Factory;
#pragma warning disable CS0618 // We are still validating Unstable API in our tests even if it's marked as obsolete.
namespace Asm6502.Tests;

[TestClass]
public partial class Mos6510AssemblerTests
{
    [TestMethod]
    public async Task ADC_Absolute()
    {
        using var asm = CreateAsm()
            .ADC(0x1234)
            .ADC(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .ADC(0x1234, X)
            .ADC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_AbsoluteY()
    {
        using var asm = CreateAsm()
            .ADC(0x1234, Y)
            .ADC(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_Immediate()
    {
        using var asm = CreateAsm()
            .ADC_Imm(0x01)
            .ADC_Imm(0x42)
            .ADC_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_IndirectX()
    {
        using var asm = CreateAsm()
            .ADC(_[0x05, X])
            .ADC(_[0x20, X])
            .ADC(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_IndirectY()
    {
        using var asm = CreateAsm()
            .ADC(_[0x06], Y)
            .ADC(_[0x30], Y)
            .ADC(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_ZeroPage()
    {
        using var asm = CreateAsm()
            .ADC(0x03)
            .ADC(0x20)
            .ADC(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ADC_ZeroPageX()
    {
        using var asm = CreateAsm()
            .ADC(0x02, X)
            .ADC(0x30, X)
            .ADC(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_Absolute()
    {
        using var asm = CreateAsm()
            .AND(0x1234)
            .AND(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_AbsoluteX()
    {
        using var asm = CreateAsm()
            .AND(0x1234, X)
            .AND(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_AbsoluteY()
    {
        using var asm = CreateAsm()
            .AND(0x1234, Y)
            .AND(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_Immediate()
    {
        using var asm = CreateAsm()
            .AND_Imm(0x01)
            .AND_Imm(0x42)
            .AND_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_IndirectX()
    {
        using var asm = CreateAsm()
            .AND(_[0x05, X])
            .AND(_[0x20, X])
            .AND(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_IndirectY()
    {
        using var asm = CreateAsm()
            .AND(_[0x06], Y)
            .AND(_[0x30], Y)
            .AND(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_ZeroPage()
    {
        using var asm = CreateAsm()
            .AND(0x03)
            .AND(0x20)
            .AND(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AND_ZeroPageX()
    {
        using var asm = CreateAsm()
            .AND(0x02, X)
            .AND(0x30, X)
            .AND(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ASL_Absolute()
    {
        using var asm = CreateAsm()
            .ASL(0x1234)
            .ASL(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ASL_AbsoluteX()
    {
        using var asm = CreateAsm()
            .ASL(0x1234, X)
            .ASL(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ASL_Accumulator()
    {
        using var asm = CreateAsm()
            .ASL(A)
            .ASL()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ASL_ZeroPage()
    {
        using var asm = CreateAsm()
            .ASL(0x03)
            .ASL(0x20)
            .ASL(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ASL_ZeroPageX()
    {
        using var asm = CreateAsm()
            .ASL(0x02, X)
            .ASL(0x30, X)
            .ASL(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BCC_Relative()
    {
        using var asm = CreateAsm()
            .BCC(0x10)
            .BCC(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BCS_Relative()
    {
        using var asm = CreateAsm()
            .BCS(0x10)
            .BCS(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BEQ_Relative()
    {
        using var asm = CreateAsm()
            .BEQ(0x10)
            .BEQ(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BIT_Absolute()
    {
        using var asm = CreateAsm()
            .BIT(0x1234)
            .BIT(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BIT_ZeroPage()
    {
        using var asm = CreateAsm()
            .BIT(0x03)
            .BIT(0x20)
            .BIT(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BMI_Relative()
    {
        using var asm = CreateAsm()
            .BMI(0x10)
            .BMI(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BNE_Relative()
    {
        using var asm = CreateAsm()
            .BNE(0x10)
            .BNE(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BPL_Relative()
    {
        using var asm = CreateAsm()
            .BPL(0x10)
            .BPL(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BRK_Implied()
    {
        using var asm = CreateAsm()
            .BRK()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BVC_Relative()
    {
        using var asm = CreateAsm()
            .BVC(0x10)
            .BVC(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task BVS_Relative()
    {
        using var asm = CreateAsm()
            .BVS(0x10)
            .BVS(-37)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CLC_Implied()
    {
        using var asm = CreateAsm()
            .CLC()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CLD_Implied()
    {
        using var asm = CreateAsm()
            .CLD()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CLI_Implied()
    {
        using var asm = CreateAsm()
            .CLI()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CLV_Implied()
    {
        using var asm = CreateAsm()
            .CLV()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_Absolute()
    {
        using var asm = CreateAsm()
            .CMP(0x1234)
            .CMP(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_AbsoluteX()
    {
        using var asm = CreateAsm()
            .CMP(0x1234, X)
            .CMP(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_AbsoluteY()
    {
        using var asm = CreateAsm()
            .CMP(0x1234, Y)
            .CMP(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_Immediate()
    {
        using var asm = CreateAsm()
            .CMP_Imm(0x01)
            .CMP_Imm(0x42)
            .CMP_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_IndirectX()
    {
        using var asm = CreateAsm()
            .CMP(_[0x05, X])
            .CMP(_[0x20, X])
            .CMP(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_IndirectY()
    {
        using var asm = CreateAsm()
            .CMP(_[0x06], Y)
            .CMP(_[0x30], Y)
            .CMP(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_ZeroPage()
    {
        using var asm = CreateAsm()
            .CMP(0x03)
            .CMP(0x20)
            .CMP(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CMP_ZeroPageX()
    {
        using var asm = CreateAsm()
            .CMP(0x02, X)
            .CMP(0x30, X)
            .CMP(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CPX_Absolute()
    {
        using var asm = CreateAsm()
            .CPX(0x1234)
            .CPX(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CPX_Immediate()
    {
        using var asm = CreateAsm()
            .CPX_Imm(0x01)
            .CPX_Imm(0x42)
            .CPX_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CPX_ZeroPage()
    {
        using var asm = CreateAsm()
            .CPX(0x03)
            .CPX(0x20)
            .CPX(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CPY_Absolute()
    {
        using var asm = CreateAsm()
            .CPY(0x1234)
            .CPY(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CPY_Immediate()
    {
        using var asm = CreateAsm()
            .CPY_Imm(0x01)
            .CPY_Imm(0x42)
            .CPY_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task CPY_ZeroPage()
    {
        using var asm = CreateAsm()
            .CPY(0x03)
            .CPY(0x20)
            .CPY(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DEC_Absolute()
    {
        using var asm = CreateAsm()
            .DEC(0x1234)
            .DEC(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DEC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .DEC(0x1234, X)
            .DEC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DEC_ZeroPage()
    {
        using var asm = CreateAsm()
            .DEC(0x03)
            .DEC(0x20)
            .DEC(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DEC_ZeroPageX()
    {
        using var asm = CreateAsm()
            .DEC(0x02, X)
            .DEC(0x30, X)
            .DEC(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DEX_Implied()
    {
        using var asm = CreateAsm()
            .DEX()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DEY_Implied()
    {
        using var asm = CreateAsm()
            .DEY()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_Absolute()
    {
        using var asm = CreateAsm()
            .EOR(0x1234)
            .EOR(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_AbsoluteX()
    {
        using var asm = CreateAsm()
            .EOR(0x1234, X)
            .EOR(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_AbsoluteY()
    {
        using var asm = CreateAsm()
            .EOR(0x1234, Y)
            .EOR(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_Immediate()
    {
        using var asm = CreateAsm()
            .EOR_Imm(0x01)
            .EOR_Imm(0x42)
            .EOR_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_IndirectX()
    {
        using var asm = CreateAsm()
            .EOR(_[0x05, X])
            .EOR(_[0x20, X])
            .EOR(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_IndirectY()
    {
        using var asm = CreateAsm()
            .EOR(_[0x06], Y)
            .EOR(_[0x30], Y)
            .EOR(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_ZeroPage()
    {
        using var asm = CreateAsm()
            .EOR(0x03)
            .EOR(0x20)
            .EOR(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task EOR_ZeroPageX()
    {
        using var asm = CreateAsm()
            .EOR(0x02, X)
            .EOR(0x30, X)
            .EOR(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task INC_Absolute()
    {
        using var asm = CreateAsm()
            .INC(0x1234)
            .INC(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task INC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .INC(0x1234, X)
            .INC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task INC_ZeroPage()
    {
        using var asm = CreateAsm()
            .INC(0x03)
            .INC(0x20)
            .INC(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task INC_ZeroPageX()
    {
        using var asm = CreateAsm()
            .INC(0x02, X)
            .INC(0x30, X)
            .INC(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task INX_Implied()
    {
        using var asm = CreateAsm()
            .INX()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task INY_Implied()
    {
        using var asm = CreateAsm()
            .INY()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JMP_Absolute()
    {
        using var asm = CreateAsm()
            .JMP(0x1234)
            .JMP(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JMP_Indirect()
    {
        using var asm = CreateAsm()
            .JMP(_[0x1234])
            .JMP(_[0xFF04])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JSR_Absolute()
    {
        using var asm = CreateAsm()
            .JSR(0x1234)
            .JSR(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_Absolute()
    {
        using var asm = CreateAsm()
            .LDA(0x1234)
            .LDA(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_AbsoluteX()
    {
        using var asm = CreateAsm()
            .LDA(0x1234, X)
            .LDA(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_AbsoluteY()
    {
        using var asm = CreateAsm()
            .LDA(0x1234, Y)
            .LDA(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_Immediate()
    {
        using var asm = CreateAsm()
            .LDA_Imm(0x01)
            .LDA_Imm(0x42)
            .LDA_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_IndirectX()
    {
        using var asm = CreateAsm()
            .LDA(_[0x05, X])
            .LDA(_[0x20, X])
            .LDA(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_IndirectY()
    {
        using var asm = CreateAsm()
            .LDA(_[0x06], Y)
            .LDA(_[0x30], Y)
            .LDA(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_ZeroPage()
    {
        using var asm = CreateAsm()
            .LDA(0x03)
            .LDA(0x20)
            .LDA(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDA_ZeroPageX()
    {
        using var asm = CreateAsm()
            .LDA(0x02, X)
            .LDA(0x30, X)
            .LDA(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDX_Absolute()
    {
        using var asm = CreateAsm()
            .LDX(0x1234)
            .LDX(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDX_AbsoluteY()
    {
        using var asm = CreateAsm()
            .LDX(0x1234, Y)
            .LDX(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDX_Immediate()
    {
        using var asm = CreateAsm()
            .LDX_Imm(0x01)
            .LDX_Imm(0x42)
            .LDX_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDX_ZeroPage()
    {
        using var asm = CreateAsm()
            .LDX(0x03)
            .LDX(0x20)
            .LDX(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDX_ZeroPageY()
    {
        using var asm = CreateAsm()
            .LDX(0x01, Y)
            .LDX(0x40, Y)
            .LDX(0xFC, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDY_Absolute()
    {
        using var asm = CreateAsm()
            .LDY(0x1234)
            .LDY(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDY_AbsoluteX()
    {
        using var asm = CreateAsm()
            .LDY(0x1234, X)
            .LDY(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDY_Immediate()
    {
        using var asm = CreateAsm()
            .LDY_Imm(0x01)
            .LDY_Imm(0x42)
            .LDY_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDY_ZeroPage()
    {
        using var asm = CreateAsm()
            .LDY(0x03)
            .LDY(0x20)
            .LDY(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LDY_ZeroPageX()
    {
        using var asm = CreateAsm()
            .LDY(0x02, X)
            .LDY(0x30, X)
            .LDY(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LSR_Absolute()
    {
        using var asm = CreateAsm()
            .LSR(0x1234)
            .LSR(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LSR_AbsoluteX()
    {
        using var asm = CreateAsm()
            .LSR(0x1234, X)
            .LSR(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LSR_Accumulator()
    {
        using var asm = CreateAsm()
            .LSR(A)
            .LSR()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LSR_ZeroPage()
    {
        using var asm = CreateAsm()
            .LSR(0x03)
            .LSR(0x20)
            .LSR(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LSR_ZeroPageX()
    {
        using var asm = CreateAsm()
            .LSR(0x02, X)
            .LSR(0x30, X)
            .LSR(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_Implied()
    {
        using var asm = CreateAsm()
            .NOP()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_Absolute()
    {
        using var asm = CreateAsm()
            .ORA(0x1234)
            .ORA(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_AbsoluteX()
    {
        using var asm = CreateAsm()
            .ORA(0x1234, X)
            .ORA(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_AbsoluteY()
    {
        using var asm = CreateAsm()
            .ORA(0x1234, Y)
            .ORA(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_Immediate()
    {
        using var asm = CreateAsm()
            .ORA_Imm(0x01)
            .ORA_Imm(0x42)
            .ORA_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_IndirectX()
    {
        using var asm = CreateAsm()
            .ORA(_[0x05, X])
            .ORA(_[0x20, X])
            .ORA(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_IndirectY()
    {
        using var asm = CreateAsm()
            .ORA(_[0x06], Y)
            .ORA(_[0x30], Y)
            .ORA(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_ZeroPage()
    {
        using var asm = CreateAsm()
            .ORA(0x03)
            .ORA(0x20)
            .ORA(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ORA_ZeroPageX()
    {
        using var asm = CreateAsm()
            .ORA(0x02, X)
            .ORA(0x30, X)
            .ORA(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task PHA_Implied()
    {
        using var asm = CreateAsm()
            .PHA()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task PHP_Implied()
    {
        using var asm = CreateAsm()
            .PHP()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task PLA_Implied()
    {
        using var asm = CreateAsm()
            .PLA()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task PLP_Implied()
    {
        using var asm = CreateAsm()
            .PLP()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROL_Absolute()
    {
        using var asm = CreateAsm()
            .ROL(0x1234)
            .ROL(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROL_AbsoluteX()
    {
        using var asm = CreateAsm()
            .ROL(0x1234, X)
            .ROL(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROL_Accumulator()
    {
        using var asm = CreateAsm()
            .ROL(A)
            .ROL()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROL_ZeroPage()
    {
        using var asm = CreateAsm()
            .ROL(0x03)
            .ROL(0x20)
            .ROL(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROL_ZeroPageX()
    {
        using var asm = CreateAsm()
            .ROL(0x02, X)
            .ROL(0x30, X)
            .ROL(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROR_Absolute()
    {
        using var asm = CreateAsm()
            .ROR(0x1234)
            .ROR(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROR_AbsoluteX()
    {
        using var asm = CreateAsm()
            .ROR(0x1234, X)
            .ROR(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROR_Accumulator()
    {
        using var asm = CreateAsm()
            .ROR(A)
            .ROR()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROR_ZeroPage()
    {
        using var asm = CreateAsm()
            .ROR(0x03)
            .ROR(0x20)
            .ROR(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ROR_ZeroPageX()
    {
        using var asm = CreateAsm()
            .ROR(0x02, X)
            .ROR(0x30, X)
            .ROR(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RTI_Implied()
    {
        using var asm = CreateAsm()
            .RTI()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RTS_Implied()
    {
        using var asm = CreateAsm()
            .RTS()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_Absolute()
    {
        using var asm = CreateAsm()
            .SBC(0x1234)
            .SBC(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .SBC(0x1234, X)
            .SBC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_AbsoluteY()
    {
        using var asm = CreateAsm()
            .SBC(0x1234, Y)
            .SBC(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_Immediate()
    {
        using var asm = CreateAsm()
            .SBC_Imm(0x01)
            .SBC_Imm(0x42)
            .SBC_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_IndirectX()
    {
        using var asm = CreateAsm()
            .SBC(_[0x05, X])
            .SBC(_[0x20, X])
            .SBC(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_IndirectY()
    {
        using var asm = CreateAsm()
            .SBC(_[0x06], Y)
            .SBC(_[0x30], Y)
            .SBC(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_ZeroPage()
    {
        using var asm = CreateAsm()
            .SBC(0x03)
            .SBC(0x20)
            .SBC(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBC_ZeroPageX()
    {
        using var asm = CreateAsm()
            .SBC(0x02, X)
            .SBC(0x30, X)
            .SBC(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SEC_Implied()
    {
        using var asm = CreateAsm()
            .SEC()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SED_Implied()
    {
        using var asm = CreateAsm()
            .SED()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SEI_Implied()
    {
        using var asm = CreateAsm()
            .SEI()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_Absolute()
    {
        using var asm = CreateAsm()
            .STA(0x1234)
            .STA(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_AbsoluteX()
    {
        using var asm = CreateAsm()
            .STA(0x1234, X)
            .STA(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_AbsoluteY()
    {
        using var asm = CreateAsm()
            .STA(0x1234, Y)
            .STA(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_IndirectX()
    {
        using var asm = CreateAsm()
            .STA(_[0x05, X])
            .STA(_[0x20, X])
            .STA(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_IndirectY()
    {
        using var asm = CreateAsm()
            .STA(_[0x06], Y)
            .STA(_[0x30], Y)
            .STA(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_ZeroPage()
    {
        using var asm = CreateAsm()
            .STA(0x03)
            .STA(0x20)
            .STA(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STA_ZeroPageX()
    {
        using var asm = CreateAsm()
            .STA(0x02, X)
            .STA(0x30, X)
            .STA(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STX_Absolute()
    {
        using var asm = CreateAsm()
            .STX(0x1234)
            .STX(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STX_ZeroPage()
    {
        using var asm = CreateAsm()
            .STX(0x03)
            .STX(0x20)
            .STX(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STX_ZeroPageY()
    {
        using var asm = CreateAsm()
            .STX(0x01, Y)
            .STX(0x40, Y)
            .STX(0xFC, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STY_Absolute()
    {
        using var asm = CreateAsm()
            .STY(0x1234)
            .STY(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STY_ZeroPage()
    {
        using var asm = CreateAsm()
            .STY(0x03)
            .STY(0x20)
            .STY(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task STY_ZeroPageX()
    {
        using var asm = CreateAsm()
            .STY(0x02, X)
            .STY(0x30, X)
            .STY(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TAX_Implied()
    {
        using var asm = CreateAsm()
            .TAX()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TAY_Implied()
    {
        using var asm = CreateAsm()
            .TAY()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TSX_Implied()
    {
        using var asm = CreateAsm()
            .TSX()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TXA_Implied()
    {
        using var asm = CreateAsm()
            .TXA()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TXS_Implied()
    {
        using var asm = CreateAsm()
            .TXS()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TYA_Implied()
    {
        using var asm = CreateAsm()
            .TYA()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ALR_Immediate()
    {
        using var asm = CreateAsm()
            .ALR_Imm(0x01)
            .ALR_Imm(0x42)
            .ALR_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ANC_Immediate()
    {
        using var asm = CreateAsm()
            .ANC_Imm(0x01)
            .ANC_Imm(0x42)
            .ANC_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ANC_2B_Immediate()
    {
        using var asm = CreateAsm()
            .ANC_2B_Imm(0x01)
            .ANC_2B_Imm(0x42)
            .ANC_2B_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ANE_Immediate()
    {
        using var asm = CreateAsm()
            .ANE_Imm(0x01)
            .ANE_Imm(0x42)
            .ANE_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ARR_Immediate()
    {
        using var asm = CreateAsm()
            .ARR_Imm(0x01)
            .ARR_Imm(0x42)
            .ARR_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_Absolute()
    {
        using var asm = CreateAsm()
            .DCP(0x1234)
            .DCP(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_AbsoluteX()
    {
        using var asm = CreateAsm()
            .DCP(0x1234, X)
            .DCP(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_AbsoluteY()
    {
        using var asm = CreateAsm()
            .DCP(0x1234, Y)
            .DCP(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_IndirectX()
    {
        using var asm = CreateAsm()
            .DCP(_[0x05, X])
            .DCP(_[0x20, X])
            .DCP(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_IndirectY()
    {
        using var asm = CreateAsm()
            .DCP(_[0x06], Y)
            .DCP(_[0x30], Y)
            .DCP(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_ZeroPage()
    {
        using var asm = CreateAsm()
            .DCP(0x03)
            .DCP(0x20)
            .DCP(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task DCP_ZeroPageX()
    {
        using var asm = CreateAsm()
            .DCP(0x02, X)
            .DCP(0x30, X)
            .DCP(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_Absolute()
    {
        using var asm = CreateAsm()
            .ISC(0x1234)
            .ISC(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .ISC(0x1234, X)
            .ISC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_AbsoluteY()
    {
        using var asm = CreateAsm()
            .ISC(0x1234, Y)
            .ISC(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_IndirectX()
    {
        using var asm = CreateAsm()
            .ISC(_[0x05, X])
            .ISC(_[0x20, X])
            .ISC(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_IndirectY()
    {
        using var asm = CreateAsm()
            .ISC(_[0x06], Y)
            .ISC(_[0x30], Y)
            .ISC(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_ZeroPage()
    {
        using var asm = CreateAsm()
            .ISC(0x03)
            .ISC(0x20)
            .ISC(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task ISC_ZeroPageX()
    {
        using var asm = CreateAsm()
            .ISC(0x02, X)
            .ISC(0x30, X)
            .ISC(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_Implied()
    {
        using var asm = CreateAsm()
            .JAM()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_12_Implied()
    {
        using var asm = CreateAsm()
            .JAM_12()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_22_Implied()
    {
        using var asm = CreateAsm()
            .JAM_22()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_32_Implied()
    {
        using var asm = CreateAsm()
            .JAM_32()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_42_Implied()
    {
        using var asm = CreateAsm()
            .JAM_42()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_52_Implied()
    {
        using var asm = CreateAsm()
            .JAM_52()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_62_Implied()
    {
        using var asm = CreateAsm()
            .JAM_62()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_72_Implied()
    {
        using var asm = CreateAsm()
            .JAM_72()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_92_Implied()
    {
        using var asm = CreateAsm()
            .JAM_92()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_B2_Implied()
    {
        using var asm = CreateAsm()
            .JAM_B2()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_D2_Implied()
    {
        using var asm = CreateAsm()
            .JAM_D2()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task JAM_F2_Implied()
    {
        using var asm = CreateAsm()
            .JAM_F2()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAS_AbsoluteY()
    {
        using var asm = CreateAsm()
            .LAS(0x1234, Y)
            .LAS(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAX_Absolute()
    {
        using var asm = CreateAsm()
            .LAX(0x1234)
            .LAX(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAX_AbsoluteY()
    {
        using var asm = CreateAsm()
            .LAX(0x1234, Y)
            .LAX(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAX_IndirectX()
    {
        using var asm = CreateAsm()
            .LAX(_[0x05, X])
            .LAX(_[0x20, X])
            .LAX(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAX_IndirectY()
    {
        using var asm = CreateAsm()
            .LAX(_[0x06], Y)
            .LAX(_[0x30], Y)
            .LAX(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAX_ZeroPage()
    {
        using var asm = CreateAsm()
            .LAX(0x03)
            .LAX(0x20)
            .LAX(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LAX_ZeroPageY()
    {
        using var asm = CreateAsm()
            .LAX(0x01, Y)
            .LAX(0x40, Y)
            .LAX(0xFC, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task LXA_Immediate()
    {
        using var asm = CreateAsm()
            .LXA_Imm(0x01)
            .LXA_Imm(0x42)
            .LXA_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_Absolute()
    {
        using var asm = CreateAsm()
            .NOP(0x1234)
            .NOP(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_AbsoluteX()
    {
        using var asm = CreateAsm()
            .NOP(0x1234, X)
            .NOP(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_3C_AbsoluteX()
    {
        using var asm = CreateAsm()
            .NOP_3C(0x1234, X)
            .NOP_3C(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_5C_AbsoluteX()
    {
        using var asm = CreateAsm()
            .NOP_5C(0x1234, X)
            .NOP_5C(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_7C_AbsoluteX()
    {
        using var asm = CreateAsm()
            .NOP_7C(0x1234, X)
            .NOP_7C(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_DC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .NOP_DC(0x1234, X)
            .NOP_DC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_FC_AbsoluteX()
    {
        using var asm = CreateAsm()
            .NOP_FC(0x1234, X)
            .NOP_FC(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_Immediate()
    {
        using var asm = CreateAsm()
            .NOP_Imm(0x01)
            .NOP_Imm(0x42)
            .NOP_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_82_Immediate()
    {
        using var asm = CreateAsm()
            .NOP_82_Imm(0x01)
            .NOP_82_Imm(0x42)
            .NOP_82_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_89_Immediate()
    {
        using var asm = CreateAsm()
            .NOP_89_Imm(0x01)
            .NOP_89_Imm(0x42)
            .NOP_89_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_C2_Immediate()
    {
        using var asm = CreateAsm()
            .NOP_C2_Imm(0x01)
            .NOP_C2_Imm(0x42)
            .NOP_C2_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_E2_Immediate()
    {
        using var asm = CreateAsm()
            .NOP_E2_Imm(0x01)
            .NOP_E2_Imm(0x42)
            .NOP_E2_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_1A_Implied()
    {
        using var asm = CreateAsm()
            .NOP_1A()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_3A_Implied()
    {
        using var asm = CreateAsm()
            .NOP_3A()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_5A_Implied()
    {
        using var asm = CreateAsm()
            .NOP_5A()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_7A_Implied()
    {
        using var asm = CreateAsm()
            .NOP_7A()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_DA_Implied()
    {
        using var asm = CreateAsm()
            .NOP_DA()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_FA_Implied()
    {
        using var asm = CreateAsm()
            .NOP_FA()
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_ZeroPage()
    {
        using var asm = CreateAsm()
            .NOP(0x03)
            .NOP(0x20)
            .NOP(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_44_ZeroPage()
    {
        using var asm = CreateAsm()
            .NOP_44(0x03)
            .NOP_44(0x20)
            .NOP_44(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_64_ZeroPage()
    {
        using var asm = CreateAsm()
            .NOP_64(0x03)
            .NOP_64(0x20)
            .NOP_64(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_ZeroPageX()
    {
        using var asm = CreateAsm()
            .NOP(0x02, X)
            .NOP(0x30, X)
            .NOP(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_34_ZeroPageX()
    {
        using var asm = CreateAsm()
            .NOP_34(0x02, X)
            .NOP_34(0x30, X)
            .NOP_34(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_54_ZeroPageX()
    {
        using var asm = CreateAsm()
            .NOP_54(0x02, X)
            .NOP_54(0x30, X)
            .NOP_54(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_74_ZeroPageX()
    {
        using var asm = CreateAsm()
            .NOP_74(0x02, X)
            .NOP_74(0x30, X)
            .NOP_74(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_D4_ZeroPageX()
    {
        using var asm = CreateAsm()
            .NOP_D4(0x02, X)
            .NOP_D4(0x30, X)
            .NOP_D4(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task NOP_F4_ZeroPageX()
    {
        using var asm = CreateAsm()
            .NOP_F4(0x02, X)
            .NOP_F4(0x30, X)
            .NOP_F4(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_Absolute()
    {
        using var asm = CreateAsm()
            .RLA(0x1234)
            .RLA(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_AbsoluteX()
    {
        using var asm = CreateAsm()
            .RLA(0x1234, X)
            .RLA(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_AbsoluteY()
    {
        using var asm = CreateAsm()
            .RLA(0x1234, Y)
            .RLA(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_IndirectX()
    {
        using var asm = CreateAsm()
            .RLA(_[0x05, X])
            .RLA(_[0x20, X])
            .RLA(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_IndirectY()
    {
        using var asm = CreateAsm()
            .RLA(_[0x06], Y)
            .RLA(_[0x30], Y)
            .RLA(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_ZeroPage()
    {
        using var asm = CreateAsm()
            .RLA(0x03)
            .RLA(0x20)
            .RLA(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RLA_ZeroPageX()
    {
        using var asm = CreateAsm()
            .RLA(0x02, X)
            .RLA(0x30, X)
            .RLA(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_Absolute()
    {
        using var asm = CreateAsm()
            .RRA(0x1234)
            .RRA(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_AbsoluteX()
    {
        using var asm = CreateAsm()
            .RRA(0x1234, X)
            .RRA(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_AbsoluteY()
    {
        using var asm = CreateAsm()
            .RRA(0x1234, Y)
            .RRA(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_IndirectX()
    {
        using var asm = CreateAsm()
            .RRA(_[0x05, X])
            .RRA(_[0x20, X])
            .RRA(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_IndirectY()
    {
        using var asm = CreateAsm()
            .RRA(_[0x06], Y)
            .RRA(_[0x30], Y)
            .RRA(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_ZeroPage()
    {
        using var asm = CreateAsm()
            .RRA(0x03)
            .RRA(0x20)
            .RRA(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task RRA_ZeroPageX()
    {
        using var asm = CreateAsm()
            .RRA(0x02, X)
            .RRA(0x30, X)
            .RRA(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SAX_Absolute()
    {
        using var asm = CreateAsm()
            .SAX(0x1234)
            .SAX(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SAX_IndirectX()
    {
        using var asm = CreateAsm()
            .SAX(_[0x05, X])
            .SAX(_[0x20, X])
            .SAX(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SAX_ZeroPage()
    {
        using var asm = CreateAsm()
            .SAX(0x03)
            .SAX(0x20)
            .SAX(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SAX_ZeroPageY()
    {
        using var asm = CreateAsm()
            .SAX(0x01, Y)
            .SAX(0x40, Y)
            .SAX(0xFC, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SBX_Immediate()
    {
        using var asm = CreateAsm()
            .SBX_Imm(0x01)
            .SBX_Imm(0x42)
            .SBX_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SHA_AbsoluteY()
    {
        using var asm = CreateAsm()
            .SHA(0x1234, Y)
            .SHA(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SHA_IndirectY()
    {
        using var asm = CreateAsm()
            .SHA(_[0x06], Y)
            .SHA(_[0x30], Y)
            .SHA(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SHX_AbsoluteY()
    {
        using var asm = CreateAsm()
            .SHX(0x1234, Y)
            .SHX(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SHY_AbsoluteX()
    {
        using var asm = CreateAsm()
            .SHY(0x1234, X)
            .SHY(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_Absolute()
    {
        using var asm = CreateAsm()
            .SLO(0x1234)
            .SLO(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_AbsoluteX()
    {
        using var asm = CreateAsm()
            .SLO(0x1234, X)
            .SLO(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_AbsoluteY()
    {
        using var asm = CreateAsm()
            .SLO(0x1234, Y)
            .SLO(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_IndirectX()
    {
        using var asm = CreateAsm()
            .SLO(_[0x05, X])
            .SLO(_[0x20, X])
            .SLO(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_IndirectY()
    {
        using var asm = CreateAsm()
            .SLO(_[0x06], Y)
            .SLO(_[0x30], Y)
            .SLO(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_ZeroPage()
    {
        using var asm = CreateAsm()
            .SLO(0x03)
            .SLO(0x20)
            .SLO(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SLO_ZeroPageX()
    {
        using var asm = CreateAsm()
            .SLO(0x02, X)
            .SLO(0x30, X)
            .SLO(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_Absolute()
    {
        using var asm = CreateAsm()
            .SRE(0x1234)
            .SRE(0xFF01)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_AbsoluteX()
    {
        using var asm = CreateAsm()
            .SRE(0x1234, X)
            .SRE(0xFF02, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_AbsoluteY()
    {
        using var asm = CreateAsm()
            .SRE(0x1234, Y)
            .SRE(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_IndirectX()
    {
        using var asm = CreateAsm()
            .SRE(_[0x05, X])
            .SRE(_[0x20, X])
            .SRE(_[0xFF, X])
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_IndirectY()
    {
        using var asm = CreateAsm()
            .SRE(_[0x06], Y)
            .SRE(_[0x30], Y)
            .SRE(_[0xFE], Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_ZeroPage()
    {
        using var asm = CreateAsm()
            .SRE(0x03)
            .SRE(0x20)
            .SRE(0xFE)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task SRE_ZeroPageX()
    {
        using var asm = CreateAsm()
            .SRE(0x02, X)
            .SRE(0x30, X)
            .SRE(0xFB, X)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task TAS_AbsoluteY()
    {
        using var asm = CreateAsm()
            .TAS(0x1234, Y)
            .TAS(0xFF03, Y)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task USBC_Immediate()
    {
        using var asm = CreateAsm()
            .USBC_Imm(0x01)
            .USBC_Imm(0x42)
            .USBC_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
    [TestMethod]
    public async Task AllInstructions()
    {
        using var asm = CreateAsm()
            .ADC(0x1234)
            .ADC(0xFF01)
            .ADC(0x1234, X)
            .ADC(0xFF02, X)
            .ADC(0x1234, Y)
            .ADC(0xFF03, Y)
            .ADC_Imm(0x01)
            .ADC_Imm(0x42)
            .ADC_Imm(0xFF)
            .ADC(_[0x05, X])
            .ADC(_[0x20, X])
            .ADC(_[0xFF, X])
            .ADC(_[0x06], Y)
            .ADC(_[0x30], Y)
            .ADC(_[0xFE], Y)
            .ADC(0x03)
            .ADC(0x20)
            .ADC(0xFE)
            .ADC(0x02, X)
            .ADC(0x30, X)
            .ADC(0xFB, X)
            .AND(0x1234)
            .AND(0xFF01)
            .AND(0x1234, X)
            .AND(0xFF02, X)
            .AND(0x1234, Y)
            .AND(0xFF03, Y)
            .AND_Imm(0x01)
            .AND_Imm(0x42)
            .AND_Imm(0xFF)
            .AND(_[0x05, X])
            .AND(_[0x20, X])
            .AND(_[0xFF, X])
            .AND(_[0x06], Y)
            .AND(_[0x30], Y)
            .AND(_[0xFE], Y)
            .AND(0x03)
            .AND(0x20)
            .AND(0xFE)
            .AND(0x02, X)
            .AND(0x30, X)
            .AND(0xFB, X)
            .ASL(0x1234)
            .ASL(0xFF01)
            .ASL(0x1234, X)
            .ASL(0xFF02, X)
            .ASL(A)
            .ASL()
            .ASL(0x03)
            .ASL(0x20)
            .ASL(0xFE)
            .ASL(0x02, X)
            .ASL(0x30, X)
            .ASL(0xFB, X)
            .BCC(0x10)
            .BCC(-37)
            .BCS(0x10)
            .BCS(-37)
            .BEQ(0x10)
            .BEQ(-37)
            .BIT(0x1234)
            .BIT(0xFF01)
            .BIT(0x03)
            .BIT(0x20)
            .BIT(0xFE)
            .BMI(0x10)
            .BMI(-37)
            .BNE(0x10)
            .BNE(-37)
            .BPL(0x10)
            .BPL(-37)
            .BRK()
            .BVC(0x10)
            .BVC(-37)
            .BVS(0x10)
            .BVS(-37)
            .CLC()
            .CLD()
            .CLI()
            .CLV()
            .CMP(0x1234)
            .CMP(0xFF01)
            .CMP(0x1234, X)
            .CMP(0xFF02, X)
            .CMP(0x1234, Y)
            .CMP(0xFF03, Y)
            .CMP_Imm(0x01)
            .CMP_Imm(0x42)
            .CMP_Imm(0xFF)
            .CMP(_[0x05, X])
            .CMP(_[0x20, X])
            .CMP(_[0xFF, X])
            .CMP(_[0x06], Y)
            .CMP(_[0x30], Y)
            .CMP(_[0xFE], Y)
            .CMP(0x03)
            .CMP(0x20)
            .CMP(0xFE)
            .CMP(0x02, X)
            .CMP(0x30, X)
            .CMP(0xFB, X)
            .CPX(0x1234)
            .CPX(0xFF01)
            .CPX_Imm(0x01)
            .CPX_Imm(0x42)
            .CPX_Imm(0xFF)
            .CPX(0x03)
            .CPX(0x20)
            .CPX(0xFE)
            .CPY(0x1234)
            .CPY(0xFF01)
            .CPY_Imm(0x01)
            .CPY_Imm(0x42)
            .CPY_Imm(0xFF)
            .CPY(0x03)
            .CPY(0x20)
            .CPY(0xFE)
            .DEC(0x1234)
            .DEC(0xFF01)
            .DEC(0x1234, X)
            .DEC(0xFF02, X)
            .DEC(0x03)
            .DEC(0x20)
            .DEC(0xFE)
            .DEC(0x02, X)
            .DEC(0x30, X)
            .DEC(0xFB, X)
            .DEX()
            .DEY()
            .EOR(0x1234)
            .EOR(0xFF01)
            .EOR(0x1234, X)
            .EOR(0xFF02, X)
            .EOR(0x1234, Y)
            .EOR(0xFF03, Y)
            .EOR_Imm(0x01)
            .EOR_Imm(0x42)
            .EOR_Imm(0xFF)
            .EOR(_[0x05, X])
            .EOR(_[0x20, X])
            .EOR(_[0xFF, X])
            .EOR(_[0x06], Y)
            .EOR(_[0x30], Y)
            .EOR(_[0xFE], Y)
            .EOR(0x03)
            .EOR(0x20)
            .EOR(0xFE)
            .EOR(0x02, X)
            .EOR(0x30, X)
            .EOR(0xFB, X)
            .INC(0x1234)
            .INC(0xFF01)
            .INC(0x1234, X)
            .INC(0xFF02, X)
            .INC(0x03)
            .INC(0x20)
            .INC(0xFE)
            .INC(0x02, X)
            .INC(0x30, X)
            .INC(0xFB, X)
            .INX()
            .INY()
            .JMP(0x1234)
            .JMP(0xFF01)
            .JMP(_[0x1234])
            .JMP(_[0xFF04])
            .JSR(0x1234)
            .JSR(0xFF01)
            .LDA(0x1234)
            .LDA(0xFF01)
            .LDA(0x1234, X)
            .LDA(0xFF02, X)
            .LDA(0x1234, Y)
            .LDA(0xFF03, Y)
            .LDA_Imm(0x01)
            .LDA_Imm(0x42)
            .LDA_Imm(0xFF)
            .LDA(_[0x05, X])
            .LDA(_[0x20, X])
            .LDA(_[0xFF, X])
            .LDA(_[0x06], Y)
            .LDA(_[0x30], Y)
            .LDA(_[0xFE], Y)
            .LDA(0x03)
            .LDA(0x20)
            .LDA(0xFE)
            .LDA(0x02, X)
            .LDA(0x30, X)
            .LDA(0xFB, X)
            .LDX(0x1234)
            .LDX(0xFF01)
            .LDX(0x1234, Y)
            .LDX(0xFF03, Y)
            .LDX_Imm(0x01)
            .LDX_Imm(0x42)
            .LDX_Imm(0xFF)
            .LDX(0x03)
            .LDX(0x20)
            .LDX(0xFE)
            .LDX(0x01, Y)
            .LDX(0x40, Y)
            .LDX(0xFC, Y)
            .LDY(0x1234)
            .LDY(0xFF01)
            .LDY(0x1234, X)
            .LDY(0xFF02, X)
            .LDY_Imm(0x01)
            .LDY_Imm(0x42)
            .LDY_Imm(0xFF)
            .LDY(0x03)
            .LDY(0x20)
            .LDY(0xFE)
            .LDY(0x02, X)
            .LDY(0x30, X)
            .LDY(0xFB, X)
            .LSR(0x1234)
            .LSR(0xFF01)
            .LSR(0x1234, X)
            .LSR(0xFF02, X)
            .LSR(A)
            .LSR()
            .LSR(0x03)
            .LSR(0x20)
            .LSR(0xFE)
            .LSR(0x02, X)
            .LSR(0x30, X)
            .LSR(0xFB, X)
            .NOP()
            .ORA(0x1234)
            .ORA(0xFF01)
            .ORA(0x1234, X)
            .ORA(0xFF02, X)
            .ORA(0x1234, Y)
            .ORA(0xFF03, Y)
            .ORA_Imm(0x01)
            .ORA_Imm(0x42)
            .ORA_Imm(0xFF)
            .ORA(_[0x05, X])
            .ORA(_[0x20, X])
            .ORA(_[0xFF, X])
            .ORA(_[0x06], Y)
            .ORA(_[0x30], Y)
            .ORA(_[0xFE], Y)
            .ORA(0x03)
            .ORA(0x20)
            .ORA(0xFE)
            .ORA(0x02, X)
            .ORA(0x30, X)
            .ORA(0xFB, X)
            .PHA()
            .PHP()
            .PLA()
            .PLP()
            .ROL(0x1234)
            .ROL(0xFF01)
            .ROL(0x1234, X)
            .ROL(0xFF02, X)
            .ROL(A)
            .ROL()
            .ROL(0x03)
            .ROL(0x20)
            .ROL(0xFE)
            .ROL(0x02, X)
            .ROL(0x30, X)
            .ROL(0xFB, X)
            .ROR(0x1234)
            .ROR(0xFF01)
            .ROR(0x1234, X)
            .ROR(0xFF02, X)
            .ROR(A)
            .ROR()
            .ROR(0x03)
            .ROR(0x20)
            .ROR(0xFE)
            .ROR(0x02, X)
            .ROR(0x30, X)
            .ROR(0xFB, X)
            .RTI()
            .RTS()
            .SBC(0x1234)
            .SBC(0xFF01)
            .SBC(0x1234, X)
            .SBC(0xFF02, X)
            .SBC(0x1234, Y)
            .SBC(0xFF03, Y)
            .SBC_Imm(0x01)
            .SBC_Imm(0x42)
            .SBC_Imm(0xFF)
            .SBC(_[0x05, X])
            .SBC(_[0x20, X])
            .SBC(_[0xFF, X])
            .SBC(_[0x06], Y)
            .SBC(_[0x30], Y)
            .SBC(_[0xFE], Y)
            .SBC(0x03)
            .SBC(0x20)
            .SBC(0xFE)
            .SBC(0x02, X)
            .SBC(0x30, X)
            .SBC(0xFB, X)
            .SEC()
            .SED()
            .SEI()
            .STA(0x1234)
            .STA(0xFF01)
            .STA(0x1234, X)
            .STA(0xFF02, X)
            .STA(0x1234, Y)
            .STA(0xFF03, Y)
            .STA(_[0x05, X])
            .STA(_[0x20, X])
            .STA(_[0xFF, X])
            .STA(_[0x06], Y)
            .STA(_[0x30], Y)
            .STA(_[0xFE], Y)
            .STA(0x03)
            .STA(0x20)
            .STA(0xFE)
            .STA(0x02, X)
            .STA(0x30, X)
            .STA(0xFB, X)
            .STX(0x1234)
            .STX(0xFF01)
            .STX(0x03)
            .STX(0x20)
            .STX(0xFE)
            .STX(0x01, Y)
            .STX(0x40, Y)
            .STX(0xFC, Y)
            .STY(0x1234)
            .STY(0xFF01)
            .STY(0x03)
            .STY(0x20)
            .STY(0xFE)
            .STY(0x02, X)
            .STY(0x30, X)
            .STY(0xFB, X)
            .TAX()
            .TAY()
            .TSX()
            .TXA()
            .TXS()
            .TYA()
            .ALR_Imm(0x01)
            .ALR_Imm(0x42)
            .ALR_Imm(0xFF)
            .ANC_Imm(0x01)
            .ANC_Imm(0x42)
            .ANC_Imm(0xFF)
            .ANC_2B_Imm(0x01)
            .ANC_2B_Imm(0x42)
            .ANC_2B_Imm(0xFF)
            .ANE_Imm(0x01)
            .ANE_Imm(0x42)
            .ANE_Imm(0xFF)
            .ARR_Imm(0x01)
            .ARR_Imm(0x42)
            .ARR_Imm(0xFF)
            .DCP(0x1234)
            .DCP(0xFF01)
            .DCP(0x1234, X)
            .DCP(0xFF02, X)
            .DCP(0x1234, Y)
            .DCP(0xFF03, Y)
            .DCP(_[0x05, X])
            .DCP(_[0x20, X])
            .DCP(_[0xFF, X])
            .DCP(_[0x06], Y)
            .DCP(_[0x30], Y)
            .DCP(_[0xFE], Y)
            .DCP(0x03)
            .DCP(0x20)
            .DCP(0xFE)
            .DCP(0x02, X)
            .DCP(0x30, X)
            .DCP(0xFB, X)
            .ISC(0x1234)
            .ISC(0xFF01)
            .ISC(0x1234, X)
            .ISC(0xFF02, X)
            .ISC(0x1234, Y)
            .ISC(0xFF03, Y)
            .ISC(_[0x05, X])
            .ISC(_[0x20, X])
            .ISC(_[0xFF, X])
            .ISC(_[0x06], Y)
            .ISC(_[0x30], Y)
            .ISC(_[0xFE], Y)
            .ISC(0x03)
            .ISC(0x20)
            .ISC(0xFE)
            .ISC(0x02, X)
            .ISC(0x30, X)
            .ISC(0xFB, X)
            .JAM()
            .JAM_12()
            .JAM_22()
            .JAM_32()
            .JAM_42()
            .JAM_52()
            .JAM_62()
            .JAM_72()
            .JAM_92()
            .JAM_B2()
            .JAM_D2()
            .JAM_F2()
            .LAS(0x1234, Y)
            .LAS(0xFF03, Y)
            .LAX(0x1234)
            .LAX(0xFF01)
            .LAX(0x1234, Y)
            .LAX(0xFF03, Y)
            .LAX(_[0x05, X])
            .LAX(_[0x20, X])
            .LAX(_[0xFF, X])
            .LAX(_[0x06], Y)
            .LAX(_[0x30], Y)
            .LAX(_[0xFE], Y)
            .LAX(0x03)
            .LAX(0x20)
            .LAX(0xFE)
            .LAX(0x01, Y)
            .LAX(0x40, Y)
            .LAX(0xFC, Y)
            .LXA_Imm(0x01)
            .LXA_Imm(0x42)
            .LXA_Imm(0xFF)
            .NOP(0x1234)
            .NOP(0xFF01)
            .NOP(0x1234, X)
            .NOP(0xFF02, X)
            .NOP_3C(0x1234, X)
            .NOP_3C(0xFF02, X)
            .NOP_5C(0x1234, X)
            .NOP_5C(0xFF02, X)
            .NOP_7C(0x1234, X)
            .NOP_7C(0xFF02, X)
            .NOP_DC(0x1234, X)
            .NOP_DC(0xFF02, X)
            .NOP_FC(0x1234, X)
            .NOP_FC(0xFF02, X)
            .NOP_Imm(0x01)
            .NOP_Imm(0x42)
            .NOP_Imm(0xFF)
            .NOP_82_Imm(0x01)
            .NOP_82_Imm(0x42)
            .NOP_82_Imm(0xFF)
            .NOP_89_Imm(0x01)
            .NOP_89_Imm(0x42)
            .NOP_89_Imm(0xFF)
            .NOP_C2_Imm(0x01)
            .NOP_C2_Imm(0x42)
            .NOP_C2_Imm(0xFF)
            .NOP_E2_Imm(0x01)
            .NOP_E2_Imm(0x42)
            .NOP_E2_Imm(0xFF)
            .NOP_1A()
            .NOP_3A()
            .NOP_5A()
            .NOP_7A()
            .NOP_DA()
            .NOP_FA()
            .NOP(0x03)
            .NOP(0x20)
            .NOP(0xFE)
            .NOP_44(0x03)
            .NOP_44(0x20)
            .NOP_44(0xFE)
            .NOP_64(0x03)
            .NOP_64(0x20)
            .NOP_64(0xFE)
            .NOP(0x02, X)
            .NOP(0x30, X)
            .NOP(0xFB, X)
            .NOP_34(0x02, X)
            .NOP_34(0x30, X)
            .NOP_34(0xFB, X)
            .NOP_54(0x02, X)
            .NOP_54(0x30, X)
            .NOP_54(0xFB, X)
            .NOP_74(0x02, X)
            .NOP_74(0x30, X)
            .NOP_74(0xFB, X)
            .NOP_D4(0x02, X)
            .NOP_D4(0x30, X)
            .NOP_D4(0xFB, X)
            .NOP_F4(0x02, X)
            .NOP_F4(0x30, X)
            .NOP_F4(0xFB, X)
            .RLA(0x1234)
            .RLA(0xFF01)
            .RLA(0x1234, X)
            .RLA(0xFF02, X)
            .RLA(0x1234, Y)
            .RLA(0xFF03, Y)
            .RLA(_[0x05, X])
            .RLA(_[0x20, X])
            .RLA(_[0xFF, X])
            .RLA(_[0x06], Y)
            .RLA(_[0x30], Y)
            .RLA(_[0xFE], Y)
            .RLA(0x03)
            .RLA(0x20)
            .RLA(0xFE)
            .RLA(0x02, X)
            .RLA(0x30, X)
            .RLA(0xFB, X)
            .RRA(0x1234)
            .RRA(0xFF01)
            .RRA(0x1234, X)
            .RRA(0xFF02, X)
            .RRA(0x1234, Y)
            .RRA(0xFF03, Y)
            .RRA(_[0x05, X])
            .RRA(_[0x20, X])
            .RRA(_[0xFF, X])
            .RRA(_[0x06], Y)
            .RRA(_[0x30], Y)
            .RRA(_[0xFE], Y)
            .RRA(0x03)
            .RRA(0x20)
            .RRA(0xFE)
            .RRA(0x02, X)
            .RRA(0x30, X)
            .RRA(0xFB, X)
            .SAX(0x1234)
            .SAX(0xFF01)
            .SAX(_[0x05, X])
            .SAX(_[0x20, X])
            .SAX(_[0xFF, X])
            .SAX(0x03)
            .SAX(0x20)
            .SAX(0xFE)
            .SAX(0x01, Y)
            .SAX(0x40, Y)
            .SAX(0xFC, Y)
            .SBX_Imm(0x01)
            .SBX_Imm(0x42)
            .SBX_Imm(0xFF)
            .SHA(0x1234, Y)
            .SHA(0xFF03, Y)
            .SHA(_[0x06], Y)
            .SHA(_[0x30], Y)
            .SHA(_[0xFE], Y)
            .SHX(0x1234, Y)
            .SHX(0xFF03, Y)
            .SHY(0x1234, X)
            .SHY(0xFF02, X)
            .SLO(0x1234)
            .SLO(0xFF01)
            .SLO(0x1234, X)
            .SLO(0xFF02, X)
            .SLO(0x1234, Y)
            .SLO(0xFF03, Y)
            .SLO(_[0x05, X])
            .SLO(_[0x20, X])
            .SLO(_[0xFF, X])
            .SLO(_[0x06], Y)
            .SLO(_[0x30], Y)
            .SLO(_[0xFE], Y)
            .SLO(0x03)
            .SLO(0x20)
            .SLO(0xFE)
            .SLO(0x02, X)
            .SLO(0x30, X)
            .SLO(0xFB, X)
            .SRE(0x1234)
            .SRE(0xFF01)
            .SRE(0x1234, X)
            .SRE(0xFF02, X)
            .SRE(0x1234, Y)
            .SRE(0xFF03, Y)
            .SRE(_[0x05, X])
            .SRE(_[0x20, X])
            .SRE(_[0xFF, X])
            .SRE(_[0x06], Y)
            .SRE(_[0x30], Y)
            .SRE(_[0xFE], Y)
            .SRE(0x03)
            .SRE(0x20)
            .SRE(0xFE)
            .SRE(0x02, X)
            .SRE(0x30, X)
            .SRE(0xFB, X)
            .TAS(0x1234, Y)
            .TAS(0xFF03, Y)
            .USBC_Imm(0x01)
            .USBC_Imm(0x42)
            .USBC_Imm(0xFF)
            .End();
        await VerifyAsm(asm);
    }
    
}
